...
get.read.dataset <-function() {
  ...
  #loading the known data set in a vector named classdata 
  ...
  classdata$order_date <- as.character(classdata$order_date) #change class from factor to charcter
...
#no more NA's in account creation date after running the above line.
  ...
  classdata$order_date <- as.factor(classdata$order_date) #convert back to factor
  ...
  classdata$deliverydate_actual <- as.Date(classdata$deliverydate_actual) #change class from factor
  #after running the above line when we run the summary(classdata$deliverydate_actual) we noticed that all the 0000/00/00 values changed to NA. 
  ...
  classdata$deliverydate_actual <- with(classdata, as.Date(ifelse((audiobook_download_count>0), 
                                                          order_date , deliverydate_actual), origin = "1970-01-01"))
  #same logic appplies to audiobook_count
  classdata$deliverydate_actual <- with(classdata, as.Date(ifelse((audiobook_count>0), 
                                                          order_date , deliverydate_actual), origin = "1970-01-01"))
  
  #same logic appplies to ebook_count
  classdata$deliverydate_actual <- with(classdata, as.Date(ifelse((ebook_count>0), 
                                                          order_date , deliverydate_actual), origin = "1970-01-01"))
  ...
  date_diff<-difftime(classdata$deliverydate_actual, classdata$order_date, units = "days")
...
#here we are calculating the inter quartile range in vector IQR. The difference between upper quartile and lower quartile is known as inter quartile range. 
  ...
  classdata$deliverydate_estimated <- as.Date(classdata$deliverydate_estimated) #change class from factor to Date
  ...
  clean_date_diff<-difftime(classdata$deliverydate_estimated, classdata$order_date, units = "days")
  ...
  #change class of form of address from factor to character
  classdata$form_of_address = as.character(classdata$form_of_address)
  #replace all NA values in form of address column with other.
  classdata$form_of_address[is.na(classdata$form_of_address)] <- "Other"
  #convert class back to factor
  classdata$form_of_address = factor(classdata$form_of_address)
  
  #change class of advertising code from factor to character
  # classdata$advertising_code = as.character(classdata$advertising_code)
  # #replace all NA values in advertising code column with other.
  # classdata$advertising_code[classdata$advertising_code==""] <- "Other"
  # classdata$advertising_code[classdata$advertising_code=="AA"] <- "Rare"
  # 
  # Find the quartile values and the inter-quantile-range IQR
